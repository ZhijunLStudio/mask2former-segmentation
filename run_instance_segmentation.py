...
def load_single_dataset(dataset_path):
    """
    加载单个数据集，包括 train 和 val。
    Args:
        dataset_path (str): 数据集路径（包含 train/trainannot 和 val/valannot 子目录）。
    Returns:
        DatasetDict: 包含 train 和 val 的数据集。
    """
    def load_images_and_annotations(image_dir, annot_dir):
        # 获取图像和标注的路径
        image_paths = sorted([os.path.join(image_dir, f) for f in os.listdir(image_dir) if f.endswith(('jpg', 'png', 'bmp'))])
        annotation_paths = sorted([os.path.join(annot_dir, f) for f in os.listdir(annot_dir) if f.endswith(('jpg', 'png', 'bmp'))])
        ...
        # 构建 Hugging Face 数据集格式
        dataset = Dataset.from_dict({"image": image_paths, "annotation": annotation_paths})
        dataset = dataset.cast_column("image", Image())  # 将图像列设置为 Image 类型
        dataset = dataset.cast_column("annotation", Image())  # 将标注列设置为 Image 类型
        return dataset
...
def load_all_datasets(root_dir):
    """
    加载多个数据集，合并 train 和 val。
    Args:
        root_dir (str): 包含多个数据集的根目录，每个子目录是一个数据集。
    Returns:
        DatasetDict: 包含所有数据集的合并数据集。
    """
    all_train_datasets = []
    all_val_datasets = []
    ...
    for dataset_name in os.listdir(root_dir):
        ...
        dataset = load_single_dataset(dataset_path)
...
def preprocess_masks(batch):
    ...
    processed_batch = {
        "image": batch["image"],
        "annotation": np.array(batch["annotation"])  # 将标注转为 numpy 数组
    }
